...
Academic finance dictates a fundamental trade-off: **signal strength (concentration)** vs. **diversification (variance reduction)**. Below is a comparison of different top quantile thresholds ($1\%$, $3\%$, $5\%$, $10\%$, and $20\%$) evaluated after liquidity slippage at a **1.0M AUM scale**:
...
| :--- | :---: | :---: |
| Standard Monthly (Net) | 0.906 | [0.423, 1.494] |
| Rolling Tranches (Net) | 0.937 | [0.453, 1.522] |
...
## 8. Systematic Research Platform: Execution Guide

### Prerequisite Environment
- Python 3.10+
- Dependencies: `pandas`, `numpy`, `matplotlib`, `statsmodels`, `pyarrow`

### Dataset Layout
Ensure the following daily data files are located in the `data/` directory:
- `data/close.parquet`: Matrix of closing prices.
- `data/volume.parquet`: Matrix of trading volumes (ADV).
- `data/universe.parquet`: Binary boolean matrix of index membership.
- `data/fama_french_5_factor.parquet`: Daily Fama-French factor returns.

### Running the Backtest Suite
To run the full asset pricing backtests, regressions, and capacity decay curves:
```bash
python main.py
```
This command executes:
1. Daily signal construction and rolling volatility estimation.
2. Index futures trend-hedging overlay calculations.
3. Multi-period Fama-French time-series regressions.
4. Block Bootstrap Sharpe ratio confidence interval estimations.
5. Large-scale capacity decay simulation (up to $50B AUM) across three routing regimes.
6. Auto-generation of performance plots (`results/figures/`) and master report (`reports/report.md`).

### Bibliography & Seminal References
